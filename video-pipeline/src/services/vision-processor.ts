...
export class VisionProcessor {
  ...
  async processModule(module: CourseModule): Promise<ProcessedModule> {
    try {
      ...
      const processedContent = await this.processWithImages(
        content.transcript,
        timestampedContent,
        template,
        module.title
  ...
  private async loadModuleContent(module: CourseModule): Promise<ModuleContent> {
    let transcript = ''
    let htmlContent = ''
    let metadata = {}
    let scenes: any[] = []
  ...
  private async extractTimestampedContent(module: CourseModule): Promise<TimestampedContent[]> {
    ...
    if (await FileUtils.directoryExists(framesDir)) {
      ...
      for (const frame of frameImages) {
        const matchingContent = timestampedContent.find(tc => 
          this.timestampsMatch(tc.timestamp, frame.timestamp)
        )
  ...
  private async findImagesForTimestamp(timestamp: string, module: CourseModule): Promise<ImageFrame[]> {
    ...
    if (await FileUtils.directoryExists(framesDir)) {
      ...
      const matchingFrames = frameImages.filter(frame => 
        this.timestampsMatch(timestamp, frame.timestamp)
      )
  ...
  private async extractFrameImages(framesDir: string): Promise<ImageFrame[]> {
    const images: ImageFrame[] = []
  ...
  private frameNumberToTimestamp(frameNumber: number): string {
  ...
  private async processWithImages(
    transcript: string,
    timestampedContent: TimestampedContent[],
    template: string,
    moduleTitle: string
  ): Promise<string> {
    try {
...
Please process this content and generate structured markdown with appropriate image placements.`
  ...
  private async saveProcessedModule(processedModule: ProcessedModule): Promise<void> {
